...
class wellnez_recent_posts_widget_slider extends WP_Widget {

        function __construct() {

            parent::__construct(
                // Base ID of your widget
                'wellnez_recent_posts_widget_slider',

                // Widget name will appear in UI
                esc_html__( 'Wellnez :: Recent Posts Slider', 'wellnez' ),

                // Widget description
                array(
                    'classname'                     => '',
                    'customize_selective_refresh'   => true,
                    'description'                   => esc_html__( 'Add Recent Posts Widget', 'wellnez' ),
                )
            );
        }
                            ...
                            while( $recentposts->have_posts(  ) ) {
								...
								if( has_post_thumbnail() ){
	                                echo '<div class="recent-post" data-overlay="gradient" data-opacity="8">';
										...
										echo '</a>';
	                                    if( $show_date ){
	                                        echo '<span class="post-date text-white"><i class="fal fa-calendar-alt"></i><a href="'.esc_url( wellnez_blog_date_permalink() ).'">'.esc_html( get_the_time( 'd F Y' ) ).'</a></span>';
	                                    }
	                            	echo '</div>';
								}
                        	}
						...
						while( $recentposts->have_posts(  ) ) {
                            ...
                            echo '<div class="recent-post media align-items-center">';
								...
								echo '<div class="media-body">';
									...
									if( $show_date ){
                                        echo '<span class="text-theme text-xs"><a href="'.esc_url( wellnez_blog_date_permalink() ).'">'.esc_html( get_the_time( 'd F, Y' ) ).'</a></span>';
                                    }
                                echo '</div>';
                        	echo '</div>';
                        }
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'post_count' ); ?>"><?php _e( 'Number of Posts to show:' ,'wellnez'); ?></label>
    ...
    } // Class wellnez_recent_posts_widget_slider ends here


    // Register and load the widget
    function wellnez_recent_posts_widget_slider() {
        register_widget( 'wellnez_recent_posts_widget_slider' );
    }
    add_action( 'widgets_init', 'wellnez_recent_posts_widget_slider' );